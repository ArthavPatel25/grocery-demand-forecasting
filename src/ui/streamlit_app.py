...
def main():
            ...
            with st.form("prediction_form"):
                ...
                with form_col1:
                    store_display = st.selectbox(
                        "Store Location", list(STORE_MAPPING.keys())
                    )
                    product_display = st.selectbox(
                        "Product", list(PRODUCT_MAPPING.keys())
                    )
                ...
                prediction_date = st.date_input(
                    "Forecast Date", value=date.today() + timedelta(days=1)
                )

                submitted = st.form_submit_button(
                    "Generate Prediction", use_container_width=True
                )
    ...
    with tab2:
        ...
        if history:
            ...
            st.info(
                "No predictions yet. Generate some predictions to see history and analytics."
            )
